...
public final class Reflections {
    ...
    public static Map<String, Field> findFields(final Class<?> clazz) {
        ...
        fieldCache.put(clazz, foundFields);
        return foundFields;
    }

    public static <T extends Annotation> Map<Field, Annotation> findFields(Class<?> cls, Class<T> annotationClass) {
        Map<Class<? extends Annotation>, Map<Field, Annotation>> annotations = fieldAnnotationCache.get(cls);
        if (annotations != null) {
            Map<Field, Annotation> annotationMap = annotations.get(annotationClass);
        ...
        for (Field field : findFields(cls).values()) {
        ...
        fieldAnnotationCache.put(cls, annotations);
        return annotationMap;
    }
